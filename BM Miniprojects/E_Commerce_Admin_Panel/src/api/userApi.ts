...
export const updateUser = async ({
  ...
  data: IUserUpdateFormType;
}): Promise<IUserUpdateFormType> => {
  const res = await fakeStoreApi.patch<IUserUpdateFormType>(
    `/users/${data.id}`,
    data
  );
  return res.data;
...
export const userAddApi = async ({
  ...
  const res = await fakeStoreApi.post<IUserType>("/users", {
...
export const getUserById = async (id: number): Promise<IUserType[]> => {
  const res = await fakeStoreApi.get<IUserType[]>(`/users/${id}`);
  return res.data;
};
