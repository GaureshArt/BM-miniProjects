...
export const Users = () => {
  ...
  const userData = useUserStore((state)=>state.filterUserData);
  ...
  return (
    ...
    <Navbar/>
    <Button onClick={()=>navigate('addUser')} variant="filled" color="geekblue" size="large"> Add User</Button>
    </div>
    <div className="p-4">
    <Table<IUserTableType> dataSource={userData} className="border rounded-lg"  components={{
        body: {
          cell: (props) => (
            <td
              {...props}
              style={{ border: '1px solid #4d4d4d', padding: '8px' }} // Inline border color
            />
          ),
        },
      }} pagination={{
    ...
    <Column
      ...
      render={(_, record: IUserTableType) => (
        <Space size="small">
          <Button variant="filled" color="magenta" onClick={()=>navigate(`editUser/${record.key}`)}>Update</Button>
          <Button
            color="red"
            variant="text"
            disabled={false}
          ...
          > Remove</Button>
          <Button color="gold" variant="dashed"  onClick={()=>navigate(`userDetails/${record.key}`)}>View Details</Button>
          <Modal
          title="Are you sure?"
          open={open}
          onOk={handleOk}
