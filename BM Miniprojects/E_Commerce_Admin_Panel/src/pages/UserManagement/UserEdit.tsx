...
export const UserEdit = () => {
    ...
    const updateFilterUserData = useUserStore((state)=>state.updateFilterUserData)
    const initialUserData = useUserStore((state)=>state.filterUserData).find((user)=>user.key===(+userId!))
    const {mutate:userUpdate,isPending} = useMutation({
        ...
        onSuccess:(data:IUserUpdateFormType,_,context)=>{
            ...
            const newData:IUserTableType = {key:data.id!,phone:data.phone,username:data.username, email:data.email,firstname:(data.firstname!),lastname:data.lastname,city:(data.city!)}
  ...
  return (
    ...
    <form onSubmit={handleSubmit(handleUpdateUser)} className="grid grid-cols-2 gap-4 border p-5 rounded-lg">
        ...
        <div className="col-span-2 flex justify-center mt-4">
          <Button disabled={isPending} htmlType="submit" color="default" variant={'solid'}> Update</Button>
        </div>
      </form>
    </div>
    </>
