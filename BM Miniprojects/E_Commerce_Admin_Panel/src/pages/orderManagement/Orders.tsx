...
export const Orders = () => {
  ...
  const cartData =
    role === Role.admin
      ? useCartStore((state) => state.cartData)
      : useCartStore((state) => state.cartData).filter(
          (cart) => cart.userId === roleId
        );
    ...
    <>
      ...
      <div className=" w-full flex justify-center items-center">
        <div className="w-full p-20  space-y-6">
          {cartData
            ? cartData.map((cart: ICartType) => (
                <div
                  key={cart.id}
                  className="grid grid-cols-6 gap-4 w-full grid-rows-1 h-[30rem] "
                >
                  <div className="col-span-4  p-6 bg-white border border-gray-300 rounded-xl shadow-md min-h-[250px] flex flex-col">
                    ...
                    <p className="text-gray-500">
                      📅 Created On: {new Date(cart.date).toDateString()}
                    </p>
                    <div className="border border-dashed border-gray-400 rounded-md p-4 mt-4 flex-1 overflow-auto  scrollbar-hide">
                      <div className="flex gap-3 h-auto">
                        {cart.products.length ? (
                          ...
                          <h1 className="text-4xl text-zinc-500 font-serif flex justify-center items-center w-full h-full ">
                            Cart is empty
                          </h1>
                        )}
                      </div>
            ...
            : Array.from({ length: 2 }).map((_, index) => (
                <div key={index} className="grid grid-cols-2 gap-4 w-full">
                  <Skeleton
                    active
                    className="p-6 border rounded-xl min-h-[200px]"
                  />
                  <Skeleton
                    active
                    className="p-6 border rounded-xl min-h-[200px]"
